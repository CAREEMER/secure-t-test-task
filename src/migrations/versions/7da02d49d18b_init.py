...
Revision ID: 7da02d49d18b
Revises: 
Create Date: 2022-10-23 23:19:02.142233
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("text", sa.Text(), nullable=False),
        sa.Column("author_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("parent_post_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("parent_comment_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.ForeignKeyConstraint(
            ["author_id"],
            ["user.id"],
        ),
    ...
    op.create_index(op.f("ix_comment_parent_post_id"), "comment", ["parent_post_id"], unique=False)
    op.create_table(
        "postupvote",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("time_created", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=True),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_comment_parent_post_id"), table_name="comment")
